...
```mermaid
graph TD
    Start["Identify highest risk-stratifying factor"] --> CheckLDL{"If LDL-C >4.9 mmol/L?"}
    CheckLDL -->|Yes| FH_Eval["Exclude secondary causes and evaluate for FH. Apply clinical diagnostic criteria for FH and consider referring for genetic testing and cascade screening."]
    CheckLDL -->|No| ASCVD["ASCVD (history of ACS [MI, unstable angina], stable IHD, ischaemic stroke, TIA, PAD, AAA, post-CABG, post-PCI)"]
    
    ASCVD -->|Yes| ASCVD_Manage["Treat with maximally-tolerated statin +/- ezetimibe. Consider PCSK9 monoclonal antibody or inclisiran if LDL-C ≥1.8 mmol/L despite maximally-tolerated statin and ezetimibe, especially for post-ACS, recurrent ASCVD, polyvascular disease, or FH."]
    ASCVD_Manage --> ASCVD_Target["Aim for LDL-C <1.4 mmol/L (<55 mg/dL) in patients with history of ACS. Aim for LDL-C <1.8 mmol/L (<70 mg/dL) in other patients, consider lower based on clinical need and tolerability."]
    
    ASCVD -->|No| FH["FH (Familial Hypercholesterolaemia)"]
    FH -->|Yes| FH_Manage["Treat with maximally-tolerated statin +/- ezetimibe. Consider PCSK9 monoclonal antibody or inclisiran if LDL-C ≥2.6 mmol/L despite maximally-tolerated statin and ezetimibe."]
    FH_Manage --> FH_Target["Consider LDL-C <1.8 mmol/L (<70 mg/dL): if age >40 years old, or additional CV risk factor(s) (e.g., DM, hypertension, smoking, subclinical atherosclerosis if investigated). Consider LDL-C <2.6 mmol/L (<100 mg/dL): if age ≤40 years old and no additional CV risk factors."]
    
    FH -->|No| DM["DM (Diabetes Mellitus)"]
    DM -->|Yes| DM_Risk{"Additional DM-specific risk factors (CKD, multiple microvascular complications, DM duration ≥10 years, or glycaemic level persistently above target despite optimal treatment)?"}
    DM_Risk -->|Yes| DM_Risk_Manage["Consider maximally-tolerated statin +/- ezetimibe."]
    DM_Risk_Manage --> DM_Risk_Target["Consider LDL-C <1.8 mmol/L (<70 mg/dL)."]
    DM_Risk -->|No| DM_NoRisk_Manage["Treat with at least a moderate-intensity statin."]
    DM_NoRisk_Manage --> DM_NoRisk_Target["Aim for LDL-C <2.6 mmol/L (<100 mg/dL), consider lower based on clinical need and tolerability."]
    
    DM -->|No| CKD["CKD (eGFR <60 mL/min/1.73m² and/or ACR ≥3 mg/mmol)"]
    CKD -->|Yes| CKD_Manage["Treat with moderate-intensity statin +/- ezetimibe in non-dialysis dependent patients. Caution with dose titration."]
    CKD_Manage --> CKD_Target["Aim for LDL-C <2.6 mmol/L (<100 mg/dL), consider lower based on clinical need and tolerability."]
    
    CKD -->|No| HighLDL["LDL-C >4.9 mmol/L (>190 mg/dL) and not FH"]
    HighLDL -->|Yes| HighLDL_Manage["Consider at least a moderate-intensity statin."]
    HighLDL_Manage --> HighLDL_Target["Consider calculating 10-year CV risk (e.g. SG-FRS) to determine LDL-C target."]
    
    HighLDL -->|No| CalcRisk["Calculate 10-year cardiovascular risk (Singapore-modified FRS)"]
    CalcRisk --> HighRisk["High risk (SG-FRS >20%)"]
    CalcRisk --> IntRisk["Intermediate risk (SG-FRS 10–20%)"]
    CalcRisk --> BorderRisk["Borderline risk (SG-FRS 5–<10%)"]
    CalcRisk --> LowRisk["Low risk (SG-FRS <5%)"]
    
    HighRisk --> HighRisk_Manage["In addition to lifestyle intervention, consider maximally-tolerated statin +/- ezetimibe."]
    HighRisk_Manage --> HighRisk_Target["Consider LDL-C <1.8 mmol/L (<70 mg/dL)."]
    
    IntRisk --> IntRisk_Manage["Consider moderate-intensity statin, especially if risk enhancers present (Box A)."]
    IntRisk_Manage --> IntRisk_Target["Consider LDL-C <2.6 mmol/L (<100 mg/dL)."]
    
    BorderRisk --> BorderRisk_Manage["Consider risk-benefit discussion for a statin if risk enhancers present (Box A)."]
    BorderRisk_Manage --> BorderRisk_Target["Consider LDL-C <3.4 mmol/L (<130 mg/dL)."]
    
    LowRisk --> LowRisk_Manage["Focus on lifestyle intervention. Consider risk-benefit discussion for a statin if LDL-C persistently >4.1 mmol/L (>160 mg/dL)."]
    LowRisk_Manage --> LowRisk_Target["Consider ideal LDL-C <3.4 mmol/L (<130 mg/dL)."]
    
    Start --> GeneralManage["Lifestyle intervention for all groups. Target any other modifiable CV risk factors."]
```
...
graph TD
    Condition{Condition}
    Condition -->|ASCVD| ASCVD_Entry["ASCVD (history of ACS [MI, unstable angina], stable IHD, ischaemic stroke, TIA, PAD, AAA, post-CABG, post-PCI)"]
    Condition -->|FH| FH_Entry["FH"]

    ASCVD_Entry -->|Yes| ASCVD_Treat["Treat with maximally-tolerated statin +/- ezetimibe"]
    ASCVD_Treat --> ASCVD_Escalate["Consider PCSK9 monoclonal antibody or inclisiran if LDL-C >= 1.8 mmol/L despite maximally-tolerated statin and ezetimibe, especially for post-ACS, recurrent ASCVD, polyvascular disease, or FH"]
    ASCVD_Escalate --> ASCVD_Targets["Aim for LDL-C <1.4 mmol/L (<55 mg/dL) in patients with history of ACS<br>Aim for LDL-C <1.8 mmol/L (<70 mg/dL) in other patients, consider lower based on clinical need and tolerability"]

    FH_Entry -->|Yes| FH_Treat["Treat with maximally-tolerated statin +/- ezetimibe"]
    FH_Treat --> FH_Escalate["Consider PCSK9 monoclonal antibody or inclisiran if LDL-C >= 2.6 mmol/L despite maximally-tolerated statin and ezetimibe"]
    FH_Escalate --> FH_Targets["Consider LDL-C <1.8 mmol/L (<70 mg/dL): if age >40 years old, or additional CV risk factor(s) (e.g., DM, hypertension, smoking, subclinical atherosclerosis if investigated)<br>Consider LDL-C <2.6 mmol/L (<100 mg/dL): if age <=40 years old and no additional CV risk factors"]
```
...
```mermaid
graph TD
    Start["LDL-C >4.9 mmol/L (>190 mg/dL) and not FH"] --> Action["Consider at least a moderate-intensity statin"]
    Start --> Risk["Consider calculating 10-year CV risk (e.g. SG-FRS) to determine LDL-C target"]
```
...
```mermaid
graph TD
    A["DM"] -->|Yes| B["Additional DM-specific risk factors (CKD, multiple microvascular complications, DM duration≥10 years, or glycaemic level persistently above target despite optimal treatment)"]
    A -->|No| C["CKD (eGFR <60 mL/min/1.73m² and/or ACR ≥3 mg/mmol)"]
    B -->|Yes| D["Consider maximally-tolerated statin +/- ezetimibe"]
    B -->|No| E["Treat with at least a moderate-intensity statin"]
    D --> F["LDL-C target: Consider LDL-C <1.8 mmol/L (<70 mg/dL)"]
    E --> G["Aim for LDL-C <2.6 mmol/L (<100 mg/dL), consider lower based on clinical need and tolerability"]
    C --> H["Treat with moderate-intensity statin +/- ezetimibe in non-dialysis dependent patients. Caution with dose titration."]
    H --> I["Aim for LDL-C <2.6 mmol/L (<100 mg/dL), consider lower based on clinical need and tolerability"]
```
...
```mermaid
graph TD
    Start{"Intermediate risk (SG-FRS 10-20%)"} --> ConsiderStatin["Consider moderate-intensity statin, especially if risk enhancers present"]
    ConsiderStatin --> Target["LDL-C target: Consider LDL-C <2.6 mmol/L (<100 mg/dL)"]
    
    RiskEnhancers["Risk Enhancers: Family history of premature ASCVD, Chronic inflammatory/autoimmune disorders (RA, SLE, HIV), Persistently elevated triglycerides (>1.7 mmol/L or 150 mg/dL), Metabolic syndrome, Premature menopause (<40 years), Severe mental illness, Abnormal ankle-brachial index (<0.9), Elevated lipoprotein(a)"] -->|Present| ConsiderStatin
```
...
```mermaid
graph LR
    A["Borderline risk (SG-FRS 5-<10%)"] --> B["Consider risk-benefit discussion for a statin if risk enhancers present"]
    B --> C["LDL-C target"]
    C --> D["Consider LDL-C <3.4 mmol/L (<130 mg/dL)"]
```
...
```mermaid
graph TD
    Start["New muscle symptoms ache, tenderness, or weakness"] --> Assess["Assess history, pattern, temp, impact, measure CK, establish need"]
    Assess --> CKCheck{"CK Level"}

    CKCheck -->|<= 3 times ULN| CK3Path
    CKCheck -->|> 3 to <= 10 times ULN| CK10Path
    CKCheck -->|> 10 times ULN| CK10PlusPath

    CK3Path --> SymptomCheck{"Symptom Severity"}
    SymptomCheck -->|Tolerable| TolerableAction["Provide reassurance and education, Continue therapy"]
    SymptomCheck -->|Intolerable| IntolerableAction["Rule out other causes, Temporarily stop statin e.g. 2-4 weeks, Monitor symptoms"]
    IntolerableAction --> Reassess1["Reassess symptoms"]
    Reassess1 --> Resolved1["Symptom resolution"]
    Reassess1 --> NoImprovement1["No improvement"]
    Resolved1 --> Restart1["Restart with reduced dose, different statin, or alternate-day dosing of atorvastatin/rosuvastatin. Monitor for recurrence."]
    NoImprovement1 --> Investigate1["Consider investigating non-statin related causes"]

    CK10Path --> StopStatin10["Rule out other causes of CK elevation, Temporarily stop statin e.g. 6 weeks, Monitor symptoms and repeat CK"]
    StopStatin10 --> Reassess2["Reassess symptoms and CK level"]
    Reassess2 --> Resolved2["Symptom resolution and normalised CK"]
    Reassess2 --> NoImprovement2["No improvement"]
    Resolved2 --> Restart2["Restart with reduced dose, different statin, or alternate-day dosing of atorvastatin/rosuvastatin. Consider ezetimibe or other non-statin lipid-lowering medications. Monitor for recurrence."]
    NoImprovement2 --> Specialist2["Consider possibility of statin-induced autoimmune myositis, or non-statin related causes. Seek specialist advice."]

    CK10PlusPath --> StopStatin10Plus["Rule out other causes of CK elevation, Stop statin and check eGFR, Refer immediately if rhabdomyolysis is suspected"]
```
