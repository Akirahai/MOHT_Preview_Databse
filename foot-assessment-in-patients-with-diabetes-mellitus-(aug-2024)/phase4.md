...
```mermaid
graph TD
    Start["Assess for active diabetic foot conditions"] --> TopConditions["Signs of moderate/severe infection (e.g. wet gangrene, deep abscess, osteomyelitis) [See IWGDF/IDSA classification]\nPresence of ischaemia with infection (regardless of severity)\nAcute Charcot neuro-osteoarthropathy [Characteristics: unilateral unexplained red hot swollen foot, numbness, changes in midfoot, X-ray findings]\nAcute limb ischaemia [severe limb hypoperfusion with onset <2 weeks, characterised by features like pain, pallor, pulselessness, poikilothermia (cold), paresthesias, or paralysis]"]
    
    TopConditions -->|If any| ReferED["Refer the patient immediately to the emergency department."]
    TopConditions -->|If none| MiddleConditions["Mild infection that has not improved despite appropriate treatment\nTissue loss (ulcer) without signs of moderate/severe infection\nSigns and symptoms of chronic limb-threatening ischaemia [objectively documented atherosclerotic peripheral arterial disease in association with ischaemic rest pain or tissue loss (ulceration or gangrene) for at least 2 weeks]"]
    
    MiddleConditions -->|If any| ReferSpecialist["Treat the patient immediately and refer for timely management by multidisciplinary diabetic foot clinic or specialist (e.g. aim for specialist review within 1–2 weeks, such as orthopaedic or vascular surgeon)."]
    MiddleConditions -->|If none| CheckRisk["Check risk factors for development of DFU and conduct risk stratification (see Recommendation 2)"]
```
...
```mermaid
graph TD
    MedicalHistory["Medical History: Previous foot ulcer or amputation OR eGFR <15 ml/min/1.73m² (3mo)"]
    Callus["Callus: Simple callus, Thick callus requiring treatment, Callus with intradermal bleeding"]
    Deformity["Deformity: Pain/impairment (Charcot, Hallux valgus, Mallet toe, Pes cavus/equinus, Prominent metatarsal heads)"]
    PAD["PAD Assessment: Absence of pedal pulse (TBPI <0.7 or ABPI ≤0.9) OR Symptoms (rest pain, claudication). Note: ABPI >0.9 may be false elevation due to calcification. Toe pressure/Doppler may be used if TBPI/ABPI unavailable."]
    Neuropathy["Neuropathy Assessment: Loss of protective sensation (10g monofilament) OR Loss of vibration (128Hz fork or neurothesiometer ≤25V) OR Symptoms (numbness, tingling, pain). Alternative tests: Ipswich touch test, pin prick test."]

    RiskStratification{Risk Stratification}

    HighRisk["HIGH RISK: Previous foot ulcer/amputation OR eGFR <15 ml/min/1.73m² (3mo) OR Callus with intradermal bleeding OR (2+ of: Deformity/Thick Callus, PAD, Neuropathy)"]
    ModerateRisk["MODERATE RISK: One of: Deformity/Thick Callus, PAD, Neuropathy"]
    LowRisk["LOW RISK: No risk factors OR Simple callus"]

    ReviewHigh["Review at least every 3 to 4 months"]
    ReviewMod["Review at least every 6 months"]
    ReviewLow["Review at least once a year"]
    Referral["Consider specialist or podiatry referral. Note: Low-risk/stable skin/toenail conditions may not need referral."]

    MedicalHistory --> RiskStratification
    Callus --> RiskStratification
    Deformity --> RiskStratification
    PAD --> RiskStratification
    Neuropathy --> RiskStratification

    RiskStratification -->|High Risk Criteria| HighRisk
    RiskStratification -->|Moderate Risk Criteria| ModerateRisk
    RiskStratification -->|Low Risk Criteria| LowRisk

    HighRisk --> ReviewHigh
    ModerateRisk --> ReviewMod
    LowRisk --> ReviewLow

    HighRisk --> Referral
    ModerateRisk --> Referral
    LowRisk --> Referral
```
...
```mermaid
graph TD
    Start{Foot Examination} --> Deformity["Deformity / Callus"]
    Start --> PAD["PAD Assessment"]
    Start --> Neuropathy["Neuropathy Assessment"]

    Deformity --> Inspect["Inspect both feet thoroughly by examining skin integrity, especially areas with callus or deformity"]
    Inspect --> LookFor["Look for deformity that increases rubbing in footwear or pressure"]
    LookFor --> Examples["Examples: Callus, Charcot neuro-osteoarthropathy, Mallet toe, Hammer toe, Claw toe, Hallux valgus, Pes cavus, Pes equinus, Prominent metatarsal heads"]

    PAD --> Palpate["Palpate pedal pulses (dorsalis pedis and posterior tibial) on each foot. Note: Complementary tests include toe pressure and doppler waveform analysis."]
    Palpate --> PulseCheck{Any pedal pulse absent?}
    PulseCheck -->|Yes| TBPI["TBPI (Toe-Brachial Pressure Index)"]
    PulseCheck -->|Yes| ABPI["ABPI (Ankle-Brachial Pressure Index)"]

    TBPI --> TBPI_Steps["1. Measure absolute systolic toe pressure of each big toe. 2. Measure systolic brachial pressure of each arm. 3. Calculate TBPI: (absolute systolic toe pressure) / (higher of systolic brachial pressure)"]

    ABPI --> ABPI_Steps["1. Measure systolic ankle pressures (at dorsalis pedis and posterior tibial) of each foot. 2. Measure systolic brachial pressure of each arm. 3. Calculate ABPI: (ankle pressure) / (higher of systolic brachial pressure)"]
    ABPI_Steps --> ABPI_Note["Note: 2023 IWGDF recommends using the lower of the ankle pressures for slightly improved diagnostic accuracy. Clinician discretion applies."]

    Neuropathy --> Mono["10 g monofilament"]
    Mono --> Mono_Steps["1. Ask patient to close both eyes. 2. Apply pressure on back of hand until buckles (baseline). 3. Assess plantar region of 1st, 3rd, 5th metatarsal heads and big toe of each foot (8 sites total). Avoid ulcers/callus/scar. 4. Ask patient to inform examiner whenever pressure is felt."]

    Neuropathy --> Fork["128 Hz tuning fork"]
    Fork --> Fork_Steps["1. Ask patient to close both eyes. 2. Touch back of hand with activated fork (baseline). 3. Place activated fork over interphalangeal joint of each big toe. 4. Ask patient to inform examiner if vibration is felt."]

    Neuropathy --> Neuro["Neurothesiometer"]
    Neuro --> Neuro_Steps["1. Turn on and touch probe to back of hand (baseline). 2. Place probe at distal tip of each big toe. 3. Begin at 0 V, increase voltage gradually until patient feels vibration. 4. Take three measurements and average the readings."]

    Neuropathy --> Alt["Alternative tests (if above not available)"]
    Alt --> Ipswich["Ipswich touch test: 1. Ask patient to close both eyes. 2. Lightly sequentially touch tips of 1st, 3rd, 5th toes of both feet for 1-2s. 3. Ask patient to inform examiner whenever touch is felt."]
    Alt --> Pin["Pin prick test: 1. Ask patient to close both eyes. 2. Apply disposable pin/toothpick just proximal to toenail of big toe with enough pressure to deform skin. 3. Ask patient to inform examiner when pinprick sensation is felt."]
```
