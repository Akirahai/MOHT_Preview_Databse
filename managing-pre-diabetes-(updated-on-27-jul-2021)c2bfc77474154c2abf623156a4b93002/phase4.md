...
```mermaid
graph TD
    Diagnosis{DIAGNOSIS OF PRE-DIABETES} --> Criteria["IFG: FPG 6.1–6.9 and 2-hG < 7.8 mmol/L OR IGT: FPG < 7.0 and 2-hG 7.8–11.0 mmol/L"]
    Diagnosis --> AllPreDM["All persons with pre-diabetes"]
    AllPreDM --> ScreenCVD["Screen for CVD risk factors, including hypertension and dyslipidaemia"]
    AllPreDM --> Lifestyle["LIFESTYLE INTERVENTION"]
    AllPreDM -.-> UnableLifestyle["For those who are unable to adopt lifestyle intervention"]
    Lifestyle --> LifestyleDetails["Educate on healthy diet and increased physical activity. For overweight/obese, aim for BMI < 23 kg/m² or 5-10% body weight loss."]
    Lifestyle --> FollowUp1["REGULAR FOLLOW-UP"]
    FollowUp1 --> FollowUp1Details["Follow up at least 6-monthly with BMI and FPG, OGTT, or HbA1c as appropriate, and screen for T2DM.** If FPG ≥ 7.0 mmol/L, 2-hG ≥ 11.1 mmol/L, or HbA1c ≥ 7%, proceed to manage as per T2DM. Screen yearly for CVD risk factors. Reinforce behavioural changes."]
    FollowUp1 --> NoImprovement["For those whose glycaemic status does not improve despite lifestyle intervention"]
    NoImprovement --> AddMetformin["ADD METFORMIN"]
    UnableLifestyle --> AddMetformin
    AddMetformin --> MetforminDetails["Measure HbA1c and renal function before initiation. Start at 250 mg twice daily and gradually increase as tolerated if required (depending on glycaemic status), up to 850 mg twice daily. Continue lifestyle intervention concurrently with metformin unless unable to do so."]
    AddMetformin --> FollowUp2["REGULAR FOLLOW-UP"]
    FollowUp2 --> FollowUp2Details["Follow up at least 6-monthly with BMI and HbA1c.†† If HbA1c ≥ 7%, proceed to manage per T2DM. If HbA1c does not improve and < 7%, consider investigating for T2DM with FPG or OGTT after a metformin washout period (such as 1–2 weeks). Review renal function at least yearly. Screen yearly for CVD risk factors. Consider monitoring vitamin B12 levels, especially for anaemia or peripheral neuropathy."]
```
