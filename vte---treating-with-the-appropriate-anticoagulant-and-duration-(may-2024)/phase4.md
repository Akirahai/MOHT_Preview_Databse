...
```mermaid
graph TD
    Start{Assess VTE Type}
    Start -->|Unprovoked| Unprovoked["Unprovoked VTE"]
    Start -->|Provoked transient| Provoked["Provoked VTE by a transient risk factor"]
    Unprovoked --> UnprovokedRisk["Estimated recurrence rate: 16% per patient-year after 2 years of stopping anticoagulation"]
    Provoked --> ProvokedRisk["Estimated recurrence rate: 3.3% per patient-year after 2 years of stopping anticoagulation"]
    UnprovokedRisk --> UnprovokedTx["Treat for at least 3 months"]
    ProvokedRisk --> ProvokedTx["Treat for at least 3 months"]
    UnprovokedTx -->|Continue| UnprovokedExtend["Consider extended treatment"]
    UnprovokedTx -->|Stop| UnprovokedAspirin["Consider low-dose aspirin unless contraindicated (first unprovoked proximal DVT/PE)"]
    ProvokedTx --> ProvokedStop["Stop treatment after 3 months"]
```
